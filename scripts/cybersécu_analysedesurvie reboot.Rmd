...
output:
  html_document:
    ...
    toc_float: true    # ✅ sommaire flottant/déroulant
    number_sections: false  # pas de numéros
    toc_depth: 3       # ✅ inclut titres, sous-titres et sous-sous-titres
---
...
La cybersécurité est aujourd’hui un enjeu majeur pour les organisations publiques et privées. Face à la multiplication des attaques informatiques — vols de données, ransomwares, intrusions — il devient essentiel de comprendre non seulement la fréquence des incidents, mais aussi leur durée avant résolution. Dans ce contexte, une analyse de survie permet d’étudier le temps écoulé entre la détection d’un incident et sa résolution, tout en tenant compte des facteurs pouvant influencer ce délai (type d’attaque, secteur d’activité, niveau de préparation, etc.). Ce type d’analyse statistique offre une approche rigoureuse pour identifier les éléments qui prolongent ou réduisent la durée de traitement des incidents de cybersécurité, et ainsi améliorer la résilience des systèmes d’information.

**Problématique : Etude des durées de la résolution de cyber-attaques**

Nous allons considérer T la fin d’une cyber-attaque. La fonction de survie S(t) correspond donc à la probabilité que la résolution de cette attaque soit terminée après le temps t.
...
cybersecurity_threat <- read_csv(way_NIELS)
...
- <u>*Country*</u> : **(qualitative)** le pays où a eu lieu l'attaque;  
- <u>*Year*</u> : **(quantitative)** l'année de l'attaque;  
- <u>*Attack Type*</u> : **(qualitative)** le type d'attaque concernée;   
- <u>*Target Industry*</u> : **(qualitative)** le type d'insdustrie victime de l'attaque;  
- <u>*Financial Loss*</u> : **(quantitative)** perte financière en million de dollars  
- <u>*Number of affected users*</u> : **(quantitative)** le nombre d'utilisateurs touchés par l'attaque;  
- <u>*Attack source*</u> : **(qualitative)** le type de personne à l'origine de l'attaque;  
- <u>*Security vulnerability type*</u> : **(qualitative)** l'origine de la vulnérabilité face à l'attaque;  
- <u>*Defense Mechanism Used*</u> : **(qualitative)** méchainsme de défense utilisé;  
- <u>*Incident Resolution Time*</u> : **(quantitative)** temps de résolution de l'attaque en heures;

Nous avons donc 6 variables qualitatives et 4 variables quantitatives.


## **B) Défintion des éléments de l'analyse de survie**
...
- Censure : manque d'informations sur les données
...
## **A) Vérification des valeurs NaN:**
...
ggplot(cybersecurity_threat, aes(x = `Incident Resolution Time (in Hours)`)) +
  ...
  labs(
    title = "Histogramme du temps de résolution des incidents",
    x = "Durée de résolution (heures)",
...
ggplot(cybersecurity_threat, aes(y = `Incident Resolution Time (in Hours)`)) +
  ...
  labs(
    title = "Boîte à moustaches du temps de résolution des incidents",
    y = "Durée de résolution (heures)"
  ) +
  theme_minimal()
...
# Corrige manuellement les éventuels NA
...
# --- COORDONNÉES DES CENTROÏDES DES PAYS ---
...
map <- ggplot() +
  ...
  geom_point(
    ...
    aes(
      ...
      text = paste0(
        ...
        "<b>Durée moyenne :</b> ", round(mean_resolution, 1), " h"
      )
    ),
    alpha = 0.85
  ) +
  ...
  labs(
    ...
    subtitle = "Taille = nombre d'attaques | Couleur = durée moyenne de résolution (heures)",
    caption = "Données : projet cyberattaques"
  ) +
  theme(
...
interactive_map <- ggplotly(
  ...
  layout(
    legend = list(
      orientation = "v",
      x = 1.05, y = 0.8,
      title = list(text = "<b>Légendes</b>")
    )
  ) %>%
  config(
    scrollZoom = TRUE,   # 🔍 permet le zoom à la molette
    displaylogo = FALSE, # supprime le logo Plotly
    modeBarButtonsToRemove = c("select2d", "lasso2d") # nettoie la barre d’outils
  )
...
### **A) Défition des fonctions et des paramètres**  
  
    

| **FONCTION** | **EXPRESSION** | **DESCRIPTION** |
|:-----------:|:-----------:|:-----------:|
| T | $T$ | La durée de la résolution de la cyber-attaque (en heures) |
| S(t) | $P(X>t)$ | Fonction de survie : la probabilité que la durée de résolution de la cyber-attaque soit supérieure à $t$ |
| h(t) | $\frac{f(t)}{S(t)}$ | Fonction de risque : la probabilité que la résolution de la cyber-attaque se termine au moment $t$, étant donné que l'attaque était encore active jusque-là (risque instantané de fin d'attaque pour les attaques encore en cours) |
| H(t) | $\int_0^t h(x)\,dx$ | Risque cumulé |
...
Etant donné que nous n'avons pas de valeurs manquantes, nous n'avons pas de censure. 
  
Mais nous pouvons en créer une artificielle, car la moyenne de temps d'une résolution d'attaque est de 14h. Nous allons donc assumer que l'entreprise meurt après 14h sans avoir résolu la cyber-attaque.  
Nous allons donc créer une censure à droite.   
  


Alors créons deux datasets, un avec la censure à 14h, l'autre sans censure (l'original pour nous).  

```{r, echo=FALSE}

temps_censure <- 14  # seuil de censure à droite (en heures)
col_time <- "Incident Resolution Time (in Hours)"

# 1) Partir du tableau complet, créer une vraie colonne numérique "time", filtrer proprement
cyber <- cybersecurity_threat %>%
  mutate(time = as.numeric(.data[[col_time]])) %>%
  filter(!is.na(time) & time > 0)

# 2) SANS CENSURE : tous les événements sont observés
df_sans_censure <- cyber %>%
  mutate(event = 1)

# 3) AVEC CENSURE : tronquer à 14 h et marquer les censures
...
#### **Illustration pour mieux comprendre la censure à droite**
...
# Durées aléatoires (entre 2h et 25h)
df_demo <- data.frame(
  id = factor(1:n),
...
ggplot(df_demo, aes(y = id)) +
  ...
  scale_color_manual(values = c("0" = "#D55E00", "1" = "#0072B2"),
                     labels = c("0" = "Censuré (|)", "1" = "Événement (❌)")) +
  scale_shape_manual(values = c("0" = 124, "1" = 4),  # | et X
                     labels = c("0" = "Censuré", "1" = "Événement")) +
  labs(
    title = "Illustration de la censure à droite (seuil = 14 h)",
    x = "Temps (heures)",
    y = "Incident",
    color = "Statut",
    shape = "Statut"
...
#### **Distribution de la donnée de survie selon la censure**


```{r, echo=FALSE}
library(ggplot2)

ggplot(df_avec_censure, aes(x = `Incident Resolution Time (in Hours)`, 
                            fill = factor(event))) +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("#009EAF", "#D55E00"),
                    labels = c("Censuré (0)", "Résolu (1)")) +
  labs(title = "Distribution des temps de résolution selon l’état de l’incident",
...
df_cum <- df_avec_censure %>%
  arrange(`Incident Resolution Time (in Hours)`) %>%
  group_by(event) %>%
  mutate(prop = cumsum(event) / n())

ggplot(df_cum, aes(x = `Incident Resolution Time (in Hours)`, 
                   y = prop, color = factor(event))) +
  geom_line(size = 1.1) +
  scale_color_manual(values = c("#009EAF", "#D55E00"),
                     labels = c("Censuré (0)", "Résolu (1)")) +
  labs(title = "Proportion cumulative d’incidents selon l’état",
       x = "Temps (heures)",
...
Regardons la distribution selon les pays.  
...
bw <- 2  # largeur des bins (ajuste selon ton échelle de temps)

ggplot(df_avec_censure %>% filter(!is.na(Country)),
  ...
  geom_histogram(position = "identity",
                 ...
                 color = "black",
                 linewidth = 0.2) +
  scale_fill_manual(values = c("0" = "#5DA5DA", "1" = "#F5A362"),
                    name = "Status",
                    labels = c("Censuré (0)", "Non censuré (1)")) +
  facet_wrap(~ Country, scales = "free_y") +
  labs(
    title = "Distribution du temps de résolution par pays",
    x = "Temps de résolution (heures)",
    y = "Nombre d’incidents",
    fill = "État"
  ) +
...
Regardons la distribution selon les industries.


```{r, echo=FALSE}
ggplot(df_avec_censure %>% filter(!is.na(`Target Industry`)),
       aes(x = `Incident Resolution Time (in Hours)`,
           fill = factor(event))) +
  geom_histogram(position = "identity",
                 alpha = 0.6,
  ...
  labs(
    title = "Distribution du temps de résolution par industrie",
    x = "Temps de résolution (heures)",
    y = "Nombre d’incidents",
    fill = "État"
  ) +
...
# Nouveau histogramme, avec la censure 

#ggplot(df_avec_censure, aes(x = time)) +
#  geom_histogram(binwidth = 1, fill = "#0B78F4", color = "black", alpha = 0.7) +
#  labs(
#    title = "Histogramme du temps de résolution des incidents (avec censure)",
#    x = "Durée de résolution (heures)",
#    y = "Nombre d'incidents"
#  ) +
 # theme_minimal()
...
## **C) Estimation des fonctions**


### **KAPLAN MEIER pour la fonction de survie S(t)**

On sélectionne uniquement la colonne de temps et on la renomme proprement.
...
# df est donc juste une copie du dataset de base où on a juste renommé la colonne de temps avec un autre nom.
...
ggplot(df_avec_plot, aes(x = time, y = surv)) +
  ...
  labs(title = "Courbe de survie Kaplan–Meier (avec censure)",
...
ggplot(df_sans_plot, aes(x = time, y = surv)) +
  geom_step(linewidth = 1.1) +
  labs(title = "Courbe de survie Kaplan–Meier (sans censure)",
       x = "Temps (heures)", y = "S(t)") +
  coord_cartesian(ylim = c(0, 1)) +
  theme_minimal(base_size = 13)
```
...
On peut faire après les courbes selon l'industrie, les pays ou encore les types de défenses utilisés etc. -> à faire lol
...
à modifier : Le biais est une mesure de l'écart entre l'estimation de Kaplan-Meier et la vraie fonction de survie. Il indique si l'estimateur surestime ou sous-estime la probabilité de survie à un moment donné. Ici, le biais est positif, cela signifie que l'estimateur de Kaplan-Meier est enclin à surestimer la probabilité de survie par rapport à la réalité.
...
# --- 1) Survie estimée par Kaplan–Meier à t ---
observed_survival <- summary(fit, times = t)$surv

# --- 2) Survie empirique "naïve" ---
# proportion d'incidents résolus avant t
...
On a le même biais mais on devait s'y attendre par rapport à la courbe de Kaplan-Meier qu'on a obtenu avant.
...
ggplot(df_H, aes(x = time, y = H, color = type, linetype = type)) +
  geom_step(linewidth = 1.1) +
  labs(title = "Risque cumulée H(t) — estimateur de Nelson–Aalen",
...
même problème, les deux courbes se confondent mais va s'y j'ai la flemme 




## **DENSITÉ DE RISQUE INSTANTÉ h(t)**
...
modèle de Cox sans covariable → la baseline hazard est l’estimateur de Breslow

L'estimateur de Breslow est obtenu à partir de l'estimateur de Kaplan-Meier. Sa formule est la suivante : -> à vérifier
...
ggplot(df_breslow, aes(x = time, y = H, color = type, linetype = type)) +
  ...
  labs(
    title = "Estimation de la fonction de risque cumulée H(t) — Méthode de Breslow",
    subtitle = "Comparaison entre jeu avec et sans censure",
    x = "Temps (heures)",
    y = "Fonction de risque cumulé H(t) normalisée"
  ) +
  scale_color_manual(values = c("Sans censure" = "#0072B2",
                                "Avec censure (14h)" = "#D55E00")) +
  scale_linetype_manual(values = c("Sans censure" = "solid",
...
```{r, echo=FALSE}
# --- 1) Concaténer les deux jeux avec un facteur de groupe
df_cens <- df_avec_censure  %>% mutate(groupe = "Censurée")
df_nonc <- df_sans_censure  %>% mutate(groupe = "Non censurée")

df_all <- bind_rows(df_cens, df_nonc) %>%
  mutate(groupe = factor(groupe, levels = c("Non censurée", "Censurée")))

# --- 2) Kaplan–Meier par groupe
fit <- survfit(Surv(time, event) ~ groupe, data = df_all)

# --- 3) Test du log-rank (équivalent à lifelines.statistics.logrank_test)
lr <- survdiff(Surv(time, event) ~ groupe, data = df_all, rho = 0)
chisq <- lr$chisq
df    <- length(lr$n) - 1
pval  <- 1 - pchisq(chisq, df)

cat(sprintf("Test du log-rank : Chi² = %.3f, df = %d, p-value = %.4g\n", chisq, df, pval))

# --- 4) Préparer un data.frame pour ggplot (courbes + IC)
s <- summary(fit)
df_plot <- data.frame(
  time  = s$time,
  surv  = s$surv,
  lower = s$lower,
  upper = s$upper,
  groupe = sub("^groupe=", "", s$strata)  # nettoie le nom des strates
)

# --- 5) Tracé Kaplan–Meier avec IC
ggplot(df_plot, aes(x = time, y = surv, color = groupe, fill = groupe, linetype = groupe)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.15, linewidth = 0) +
  geom_step(linewidth = 1.1) +
  labs(
    title = "Fonction de survie (Kaplan–Meier) - Log Rank",
    #subtitle = sprintf("Test du log-rank : Chi² = %.2f, p = %.3g", chisq, pval),
    x = "Temps (heures)",
    y = "Probabilité de survie",
    color = "Groupe", fill = "Groupe", linetype = "Groupe"
  ) +
  scale_color_manual(values = c("Non censurée" = "#0072B2", "Censurée" = "#D55E00")) +
  scale_fill_manual (values = c("Non censurée" = "#0072B2", "Censurée" = "#D55E00")) +
  scale_linetype_manual(values = c("Non censurée" = "solid", "Censurée" = "dashed")) +
  theme_minimal(base_size = 13) +
  theme(
    legend.title = element_blank(),
    plot.title = element_text(face = "bold", hjust = 0.5),
...
### **COX (semi-paramétrique) -> j'ai pas compris encore**




## **C) Estimation paramétrique (ou non paramétrique, jsp dans notre cas pour l'instanbt)**
